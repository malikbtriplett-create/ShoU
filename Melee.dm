...
mob/proc/IncreaseHealth(n = 0)
	...
	if(client)
		if(lastResourceUpdate < world.time)
			if(resourceCallback) resourceCallback.Call()
...
proc/Defense_damage_reduction(mob/attacker,mob/defender)
	...
	//decrease by Speed mod
	mod -= defender.GetStatMod("Spd")
...
mob/proc/GetMeleeDamage(mob/target, dmg = Mechanics.GetSettingValue("Base Melee Damage"))
	if(ismob(target))
		...
		// Normal sustained Block.
		// Perfect Block is resolved separately before melee effects.
		if(target.IsBlocking())
			dmg *= target.GetMeleeBlockMultiplier()
...
mob/proc/GetMeleeBlockMultiplier()
	if(!blocking) return 1

	// Normal Block uses effective Durability.
	// Each point provides 10% additional melee damage reduction.
	// Active Block mitigation is capped at 60%.
	var/block_reduction = Math.Clamp(GetEffectiveStat("Durability") * 0.1, 0, 0.6)

	return 1 - block_reduction

mob/proc/ResolveMeleeBlock(mob/attacker, knockback = 0)
    ...
    if(block_started_at > 0 && block_age >= 0 && block_age <= perfect_window)
        // Consume the reaction window.
        block_started_at = 0

        // Perfect Block completely stops this melee attack.
        // Perfect Block baseline knockback.
        // A successful melee Perfect Block always launches the attacker 5 tiles.
        attacker.Knockback(src, 5)
    ...
    // The player remains in normal sustained Block.
    block_started_at = 0
...
mob/proc/GetAttackAccuracy(mob/target, dmg = 1)
	...
	var/accReduction = target.GetStatMod("Spd")
	if(src.effectiveBPTier < target.effectiveBPTier)
...
mob/proc/combo_teleport(mob/m)
	...
	if(t&&isturf(t)&&!t.density&&t.Enter(src)&&!(locate(/mob) in t))
		...
		dir=get_dir(src,m)
		if(ismob(m) && prob(50 - m.GetStatMod("Spd") + src.GetStatMod("Acc")))
			m.dir = src.dir
...
mob/proc/Melee(obj/O, from_auto_attack, force_power_attack, lunge_allowed = 0, obj/items/Weapon/weapon, test_damage = 0)
	set waitfor=0
	if(grabber) return
	...
	var/dmg = test_damage > 0 ? test_damage : GetMeleeDamage(target)
	...
	if(hit_landed)
		// BLOCK_PHASE_1_PERFECT_BLOCK_HOOK
		if(ismob(target))
			var/block_result = target.ResolveMeleeBlock(src, knockback)
