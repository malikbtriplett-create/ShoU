...
mob
	...
	proc
		...
		DragonRushPointsToWin(mob/b)
			if(!b) return 0
			var/mult = GetTierDamageMod(src, b) * 10 + b.GetStatMod("Spd") - src.GetStatMod("Spd")
			if(Class == "Legendary") mult *= 0.5
			mult = Math.Max(mult, 0.0001)
			return ToOne(6 / mult)
...
proc
	...
	DragonRushLoop(mob/a,mob/b)
		...
		var/dr_dmg = winner.GetMeleeDamage(loser, 10)
		var/base_stun = 60

		if(loser.Class == "Legendary")
			base_stun *= 1.5
			dr_dmg *= 1.5
		
		var/stun = base_stun + winner.GetStatMod("Str") * winner.GetTierBonus(0.5)
		stun -= loser.GetStatMod("Res") * loser.GetTierBonus(0.75)
		stun = Clamp(stun, 0, base_stun * 2)

		loser.TakeDamage(dr_dmg, winner)
