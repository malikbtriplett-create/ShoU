...
mob/proc
	StopBeaming()
		set waitfor=0
		for(var/obj/Skills/Combat/Ki/A in src) if(A.Wave && (A.charging || A.streaming))
	...
	KikohoDamageLoop()
		...
		while(kikoho_damage > 0)
			var/dmg_min = 2 //minimum damage per second
			var/dmg = dmg_min * RegenMod() * (kikoho_damage / kikoho_self_dmg)
			if(dmg < dmg_min) dmg = dmg_min
			TakeDamage(dmg, "Kikoho")
			kikoho_damage -= dmg
			sleep(10)
	...
	KikohoDamageTick()
		var/dmg_min = 1, dmg = dmg_min * RegenMod() * (kikoho_damage / kikoho_self_dmg)
		dmg = Math.Max(dmg, dmg_min)
		kikoho_damage -= dmg
		kikoho_damage = Math.Max(kikoho_damage, 0)

	KikohoDamageTo(mob/m)
		var/dmg = 27 + (GetStatMod("For") * GetTierBonus(0.6) - m.GetStatMod("Res") * m.GetTierBonus(0.8))
		if(Race == "Human") dmg *= 1.5
		return dmg

	FireKikoho(obj/Skills/Combat/Ki/Kikoho/k)
		...
		var/chargeup_time = KikohoRefire(0.5)
		var/elapsed_time = 0
		var/interrupted
		...
		if(!interrupted)
			...
			if(m && !cant_blast(ignore_attack_check = 1))
				dir = get_dir(src,m)
				IncreaseKi(-GetSkillDrain(mod = k.Drain, is_energy = 1))
...
obj/Kikoho_Effects
	...
	Kikoho_Rock
		...
		proc
			...
			KikohoRockFlyOff()
				var/dist = rand(3,12)
				var/_dir = pick(NORTH,SOUTH,EAST,WEST,NORTHEAST,NORTHWEST,SOUTHWEST,SOUTHEAST)
				var/spd = rand(87,113) / 100
