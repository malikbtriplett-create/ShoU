...
mob/proc
	...
	SyncActiveBuffStatModifiers()
		RemoveAllStatModifierSources("buff")
		for(var/obj/Skills/Buff/B in src)
			if(B.suffix == "Active")
				SetStatModifierSource("buff", list("Strength" = B.buff_str, "Durability" = B.buff_dur, "Speed" = B.buff_spd, "Force" = B.buff_for, "Resistance" = B.buff_res, "Accuracy" = B.buff_off), "\ref[B]", "active_buff")
	...
	Buff_Enable(obj/Skills/Buff/O) if(!O.being_edited&&!Redoing_Stats)
		...
		Off+=O.buff_off
		Def+=O.buff_def
		SetStatModifierSource("buff", list("Strength" = O.buff_str, "Durability" = O.buff_dur, "Speed" = O.buff_spd, "Force" = O.buff_for, "Resistance" = O.buff_res, "Accuracy" = O.buff_off), "\ref[O]", "active_buff")
	...
	Buff_Disable(obj/Skills/Buff/O) if(O&&O.suffix)
		...
		Off-=O.buff_off
		Def-=O.buff_def
		RemoveStatModifierSource("buff", "\ref[O]")
