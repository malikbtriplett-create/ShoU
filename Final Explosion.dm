...
obj/Skills/Combat/Ki/Final_Explosion
	teachable = 1
	Skill = 1
	Cost_To_Learn = 8
	Teach_Timer = 5
	...
	proc
		Final_Explosion()
			...
			if(!charging_final_explosion && !using_final_explosion)
				...
				if(IsFusion())
					src << "Fused characters can not use this!"
					return
		...
		BeginChargingFinalExplosion()
			...
			AlterInputDisabled(1)
			final_explosion_dmg = 30

			//mild knockaway and stun
			for(var/mob/m in view(2,src)) if(m != src)
				PowerupKnockbackEffect(m)
				var/stun = 11 + (GetStatMod("For") * GetTierBonus(0.5) * (1 + Eff / 2) - m.GetStatMod("Res") * m.GetTierBonus(0.75) * (1 + m.recov / 2))
				stun = Clamp(stun, 0, 60)
		...
		FinalExplosionDamage()
			set waitfor=0
			var/explosion_range = 1 + sqrt(final_explosion_dmg) * 0.3 * final_explosion_size_mod
			explosion_range = Clamp(explosion_range, 1, 999)
			var/wall_break_power = WallBreakPower()
			var/user_bp = BP
			var/user_force = GetStatMod("For")
			sleep(8)
		...
		FinalExplosionDamage(mob/user, dmg_percent = 0, wait_time = 0, wall_break_power = 1, user_bp = 1, user_force = 1)
			...
			for(var/v in 1 to max_stacks)
				for(var/mob/m in src)
					if(m.IsTournamentFighter()) continue
					if(viewable(user, m))
						if(v == 1) RockExplode(m.loc)

						var/dmg = dmg_percent / max_stacks * (user_bp / m.BP)**bp_exponent * (user_force / m.Res)**0.5
						if(m.IsShielding()) m.IncreaseKi(-(dmg * m.ShieldDamageReduction() * (m.max_ki / 100)))
						else m.TakeDamage(dmg, user)
